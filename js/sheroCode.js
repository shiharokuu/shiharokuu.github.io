...
xmlhttp.onreadystatechange = function() {
  ...
  if (this.readyState == 4 && this.status == 200) {
	...
	for (i of skill) {
		if(j==0){
			sheroSkillsName = sheroSkillsName + "<button class=\"\ tablinks\"\ onclick=\"\openTabs(event, '" + i.shortName + "')\"\ id=\"\defaultOpen\"\>" + i.shortName + "</button>"
			j+=1
		}
		else{
			sheroSkillsName = sheroSkillsName + "<button class=\"\ tablinks\"\ onclick=\"\openTabs(event, '" + i.shortName + "')\"\>" + i.shortName + "</button>"
		}
