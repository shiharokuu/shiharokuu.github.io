/*Firma by Damian/Shiharoku/Kenta
	Aquí se ponen los paths a las imagenes y música, además de las citas de personaje. 
	Se pueden poner ciertas citas para solo ciertas imagenes, o simplemente dejarlo todo al azar.*/ 
	var mp3 =[
	  'pj-media/music/conan.mp3',
	  'pj-media/music/hyouka1.mp3',
	  'pj-media/music/hyouka2.mp3',
	  'pj-media/music/hyouka3.mp3',
	  'pj-media/music/hyouka4.mp3',
	  'pj-media/music/hyouka5.mp3',
	  'pj-media/music/saddonokyoku.mp3',
	  'pj-media/music/believer.mp3'
	  
      ];
/*

	var pjIMG =
	  [
      'pj-media/gifs/take01.gif',
	  'pj-media/gifs/take02.gif',
	  'pj-media/gifs/take03.gif',
	  'pj-media/gifs/take04.gif',
	  'pj-media/gifs/take05.gif',
	  'pj-media/gifs/take06.gif',
	  'pj-media/gifs/take07.gif',
	  'pj-media/gifs/take08.gif',
	  'pj-media/gifs/take09.gif',
	  'pj-media/gifs/take10.gif',
	  'pj-media/gifs/take11.gif',
	  'pj-media/gifs/take12.gif',
	  'pj-media/gifs/take13.gif',
	  'pj-media/gifs/take14.gif',
	  'pj-media/gifs/take15.gif',
	  'pj-media/gifs/take16.gif',
	  'pj-media/gifs/take17.gif'
	  

      ];
*/      
var pjIMG =[
		//"If you don't want a sarcastic answer, don't ask a stupid question"
		  [
		  'pj-media/gifs/oreki-sigh.gif',
		  'pj-media/gifs/oreki-sarcasm.gif',
		  'pj-media/gifs/oreki-depre.gif',
		  'pj-media/gifs/oreki-lazy.gif',
		  'pj-media/gifs/oreki-neutral.gif'
		  ],

		//"If I don't have to do it, I won't. If I have to do it, I'll make it quick"
		  [
			'pj-media/gifs/oreki-sigh.gif',
			'pj-media/gifs/oreki-cherry.gif',
			'pj-media/gifs/oreki-yawn.gif',
			'pj-media/gifs/oreki-lazy.gif'
		  ],

		//"There is always only one truth"
		[
			'pj-media/gifs/oreki-think1.gif',
			'pj-media/gifs/oreki-think2.gif',
			'pj-media/gifs/oreki-neutral.gif'
		],

		//"When you have eliminated all which is impossible. Whatever remains, must be the truth",
		[
		'pj-media/gifs/oreki-think1.gif',
		'pj-media/gifs/oreki-think2.gif'
		],

		//"Data! Data! Data! I can't make bricks without clay."
		[
			'pj-media/gifs/oreki-think1.gif',
			'pj-media/gifs/oreki-think2.gif'
			],		

		//"You see, but you do not observe. The distinction is clear"
		[
			'pj-media/gifs/oreki-think2.gif',
			'pj-media/gifs/oreki-depre.gif'
		],		

		//"Don't change so people will like you. Be yourself and the right people will like the real you"
		[
			'pj-media/gifs/oreki-blush.gif',
			'pj-media/gifs/oreki-depre.gif',
			'pj-media/gifs/oreki-lazy.gif',
			'pj-media/gifs/oreki-neutral.gif'
		],		

		//	"Si estas leyendo esto, debes darme dinero. (?)"
		[
			'pj-media/gifs/oreki-smug.gif'
		]	
	  ];

	var pjQuotes = [
    	"If you don't want a sarcastic answer, don't ask a stupid question?",
		"If I don't have to do it, I won't. If I have to do it, I'll make it quick",
		"There is always only one truth",
		"When you have eliminated all which is impossible. Whatever remains, must be the truth",
		"Data! Data! Data! I can't make bricks without clay.",
		"You see, but you do not observe. The distinction is clear",
		"Don't change so people will like you. Be yourself, the right people will like the real you",
		"Si estas leyendo esto, debes darme dinero. (??)"
	
  ];
